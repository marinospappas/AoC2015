...
abstract class InstructionSet {
    ...
    companion object {
        ...
        val opCodesList: MutableMap<String, OpCode> = mutableMapOf(
            ...
            SUB to OpCode(MUL, 3, listOf(R, R, W)) { a -> Pair(SET_MEMORY, listOf(a[2], a[0] as Long * a[1] as Long)) },
            INC to OpCode(INC, 2, listOf(R, W)) { a -> Pair(SET_MEMORY, listOf(a[1], a[0] as Long + 1L)) },
            INC to OpCode(DEC, 2, listOf(R, W)) { a -> Pair(SET_MEMORY, listOf(a[1], a[0] as Long - 1L)) },
